...
func (r FutureEstimateFeeResult) Receive() (btcutil.Amount, error) {
	res, err := receiveFuture(r)
	if err != nil {
		return 0, err
	}
...
func (c *Client) EstimateFee(numBlocks int64) (btcutil.Amount, error) {
...
func (r FutureEstimateSmartFeeResult) Receive() (*btcjson.EstimateSmartFeeResult, error) {
	...
	// Unmarshal result as a estimatesmartfee result object.
	var smartFeeRes btcjson.EstimateSmartFeeResult
	err = json.Unmarshal(res, &smartFeeRes)
	if err != nil {
		return nil, err
	}

	return &smartFeeRes, nil
}
...
// SetTxFee sets an optional transaction fee per KB that helps ensure
// transactions are processed quickly.  Most transaction are 1KB.
func (c *Client) EstimateSmartFee(numBlocks int64) (*btcjson.EstimateSmartFeeResult, error) {
...
func (c *Client) SendToAddressCommentAsync(address btcutil.Address,
	...
	commentTo string,
	substractfee bool) FutureSendToAddressResult {

	addr := address.EncodeAddress()
	cmd := btcjson.NewSendToAddressCmd(addr, amount.ToBTC(), &comment,
		&commentTo, &substractfee)
	return c.sendCmd(cmd)
}
...
// NOTE: This function requires to the wallet to be unlocked.  See the
// WalletPassphrase function for more details.
func (c *Client) SendToAddressComment(address btcutil.Address, amount btcutil.Amount, comment, commentTo string, substractfee bool) (*chainhash.Hash, error) {
	return c.SendToAddressCommentAsync(address, amount, comment,
		commentTo, substractfee).Receive()
}
